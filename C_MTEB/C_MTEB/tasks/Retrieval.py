...
def load_local_retrieval_data(hf_hub_name, eval_splits):
    LOCAL_DIR = "/data/disk2/zhangzh/datasets"
    eval_split = eval_splits[0]
    dataset = load_dataset(os.path.join(LOCAL_DIR, hf_hub_name))
    qrels = load_dataset(os.path.join(LOCAL_DIR, hf_hub_name + "-qrels"))[eval_split]
...
class T2Retrieval(AbsTaskRetrieval):
    ...
    def load_data(self, **kwargs):
        ...
        self.corpus, self.queries, self.relevant_docs = load_local_retrieval_data(
            self.description["hf_hub_name"], self.description["eval_splits"]
        )
